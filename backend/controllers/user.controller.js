...
export const followOrUnfollow = async (req,res) => {
  try {
    ...
    const jiskoFollowkrenge =req.params.id; // friend
...
return res.staus(200).json({message:'Unfollowed successfully',success:true});
}else{
  //follow logic
  await Promise.all([
  ...
  return res.staus(200).json({message:'followed successfully',success:true});
}
  } catch (error) {
    console.log(error);
}
